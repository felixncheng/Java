...
public class DynamicToken {
    ...
    public static void main(String[] args) {
        String serect = "LIXS7JCCINANMYBL";
        DynamicToken dt = new DynamicToken(serect);
        try {
            String dynamicCode = dt.getDynamicCode();
            System.out.println(dynamicCode);
            GoogleAuthenticator googleAuthenticator = new GoogleAuthenticator();
            boolean b = googleAuthenticator.checkCode(serect, Long.valueOf(dynamicCode), System.currentTimeMillis());
    ...
    private byte[] sha1(String secret, long msg) throws Exception {
        ...
        byte[] decode = codec.decode(secret);
        SecretKey secretKey = new SecretKeySpec(decode, "");//创建秘钥
        try {
